...
static int init_matrix(matrix *a, const int columns, const int rows, double (*generator)(const double, const double))
{
	...
	if (generator != NULL) {
#pragma omp parallel for default(none) shared(a, generator) firstprivate(min_value, max_value, columns, rows)
...
static void transpose(const matrix a, matrix b)
{
	for (int j = 0; j < b.rows; ++j) {
		for (int i = 0; i < b.cols; ++i) {
			b.data[index(i, j, b.cols)] = a.data[index(j, i, a.cols)];
		}
	}
}

static void multiply(const matrix a, const matrix b_trans, matrix c)
{
	double tmp;
#pragma omp parallel for default(none) shared(a, b_trans, c) private(tmp)
	for (int j = 0; j < c.rows; ++j) {
		for (int i = 0; i < c.cols; ++i) {
			tmp = 0;
			for (int k = 0; k < a.cols; ++k) {
				tmp += a.data[index(k, j, a.cols)] * b_trans.data[index(k, i, b_trans.cols)];
			}
...
int main(int argc, char **argv)
{
	...
	matrix b_tr = {.cols = 0, .rows = 0, .data = NULL};
	...
	time(&t_start);
	transpose(b, b_tr);
	// to benchmark fair: count allocation and destruction of transposed matrix
	if (init_matrix(&b_tr, a_columns, b_columns, gen_num)) goto out;
	free(b.data);
	b.data = NULL; // don't free twice
	multiply(a, b_tr, c);
	time(&t_end);
...
out:
	...
	if(b_tr.data != NULL) free(b_tr.data);
